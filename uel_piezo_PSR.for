...
C 	  write(*,*) 'Eyoung     =', props(1)
C 	  write(*,*) 'poisson     =', props(2)
C 	  write(*,*) 'E31     =', props(3)
C 	  write(*,*) 'E32     =', props(4)
C 	  write(*,*) 'E33     =', props(5)
C 	  write(*,*) 'E24     =', props(6)
C 	  write(*,*) 'E15     =', props(7)
C 	  write(*,*) 'eps11     =', props(8)
C 	  write(*,*) 'eps22     =', props(9)
C 	  write(*,*) 'eps33     =', props(10)
      
      
      ! Construct the elastic stiffness "C" matrix
	  ! (6x6 Voigt representation of a fourth-order tensor)
	  !
	  call buildCmatVoigt(Eyoung, poisson, CmatVoigt)
...
!*****************************************************************
	  ...
	  !
	  call buildCmat(Eyoung, poisson, Cmat)
...
****************************************************************************
	  ...
	  subroutine buildCmatVoigt(Eyoung, poisson, Cmat)
	  ...
	  real*8 Cmat(6,6), Eyoung, poisson, prefac
     ...
     +     half=1.d0/2.d0)
	  ...
	  ! premultiplication factor for every entry
	  !
	  prefac = Eyoung/((one+poisson)*(one-two*poisson))
	  ...
	  !
	  do i=1,3
		    Cmat(i,i) = prefac*(one-poisson)
		    Cmat(i+3,i+3) = half*prefac*(one-two*poisson)
	  end do
	  !
	  ! off-diagonal entries
	  !
	  Cmat(1,2) = prefac*poisson
	  Cmat(1,3) = prefac*poisson
	  Cmat(2,1) = prefac*poisson
	  Cmat(2,3) = prefac*poisson
	  Cmat(3,1) = prefac*poisson
	  Cmat(3,2) = prefac*poisson
...
****************************************************************************
	  ...
	  subroutine buildCmat(Eyoung, poisson, Cmat)
	  ...
	  real*8 Cmat(3,3,3,3), Eyoung, poisson, prefac, iden(3,3)
     ...
     +     half=1.d0/2.d0)
	  ...
	  ! premultiplication factors for every entry
	  !
	  prefac1 = (Eyoung*poisson)/((one+poisson)*(one-two*poisson))
	  prefac2 = Eyoung/(two*(one+poisson))
     ...
     +                 + prefac1*iden(i,j)*iden(k,l) 
     +                 + prefac2*(iden(i,k)*iden(j,l) + iden(i,l)*iden(j,k))
			   enddo
            enddo
         enddo
      enddo
